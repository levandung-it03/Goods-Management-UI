...
export class AxiosActions {
    ...
    static getStoredAuthToken(tokenName) {
        ...
        if (!token.includes('Bearer ')) return 'Bearer ' + token;
        else return token;
    }
    ...
    static authorizedResponse = {
        ...
        catching: async (error) => {
            if (!error.config._retry) {
                try {
                    if (
                        !UtilMethods.checkIsBlank(error.response) &&
                        ...
                        if (!newAccessToken.includes('Bearer ')) newAccessToken = 'Bearer ' + newAccessToken;
...
springService.interceptors.request.use(AxiosInterceptors.authorizedRequest.request, AxiosInterceptors.authorizedRequest.catching);
springService.interceptors.response.use(AxiosInterceptors.authorizedResponse.response, AxiosInterceptors.authorizedResponse.catching);
