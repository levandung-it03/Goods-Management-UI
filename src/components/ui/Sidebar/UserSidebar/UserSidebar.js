...
function UserSidebar({ children }) {
    ...
    const jwtClaims = UtilAxios.checkAndReadBase64Token(
        cookieHelpers.getCookies().accessToken
    );
    ...
    const handleOpenProfileDialog = useCallback(
        () => setDialogContent(<UserProfileDialog userProfile={userProfile} />),
        [userProfile]
    );
    ...
    return (
        <aside className="sidebar">
            <div className="logo-container center">
                ...
                <button
                    className="toggle-sidebar center"
            ...
            <div className="item-container">
                <ul>
                    {Children.map(children, (child) =>
                        cloneElement(child, { isExpanded })
                    )}
                </ul>
            </div>
            <div className="logout-container center" onClick={handleLogout}>
                <LogOut className="logout-icon" />
                <span className={`logout${isExpanded ? " expand" : ""}`}>
                    Log out
                </span>
            </div>
            <div className="divider"></div>
            <div
                className="profile-container center"
